...
/*
template <typename N, typename E>
std::shared_ptr<Edge> gdwg::Graph<N, E>::GetFirstEdge() const noexcept {
    ...
    auto node = *node_ite;
    auto edge_ite = node->out_edges_.cbegin();
    
    // There are no out edges in the graph
    if (edge_ite == node->out_edges_.cend()) {
        return nullptr;
    }
